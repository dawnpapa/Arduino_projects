...
//#include <SPI.h>
#include <SoftwareSerial.h> //For Serial communication b/w NodeMCU and Arduino

SimpleTimer timer;

SoftwareSerial s(7,6); //(Rx, Tx) pin assign for serial communication
...
const char* ntpServerName = "time.google.com"; //"time.bora.net"; 
...
//const char* Dongne="동탄";//먼지정보 제공할 지역
...
*/
    ...
    Serial.println("setup wifi configuration");
...
//wifi connection 성공하면 Serial 날림
    if(s.available()>0){
      s.write("Connected to WiFi");
    }
...
void check_kma(){
  ...
  char time_string[20]; 
   ...
   //동네예보시간고려 
   t=t+60*30; //10분이전
   t=t-60*60*((hour(t)%3+1)%3); //02,05,08,11,14,17,20,23시에 갱신
   sprintf(base_datef, "%d%02d%02d", year(t), month(t), day(t)); 
   sprintf(base_timef, "%02d00", hour(t));
...
//------------------------동네현황 ----
 ...
 //       url += "&ServiceKey=SERVICE_KEY"; 
        ...
        url += "&nx=61"; 
        url += "&ny=119"; //화성시 반월동 61,119, 서초구양재동 61,125, 동백 63,120 신길3동 59,125 내손2동 60,123
        url += "&numOfRows=100"; 
        url += "&pageNo=1"; 
        url += "&_type=json"; 
//----------------동네예보--
       String url2 = "http://newsky2.kma.go.kr/service/SecndSrtpdFrcstInfoService2/ForecastSpaceData?"; 
        url2 += "ServiceKey="+ServiceKey; 
 //       url += "&ServiceKey=SERVICE_KEY"; 
        url2 += "&base_date="+String(base_datef); 
        url2 += "&base_time="+String(base_timef); 
        url2 += "&nx=61"; 
        url2 += "&ny=119"; //화성시 반월동 61,119, 서초구양재동 61,125, 동백 63,120 신길3동 59,125
        url2 += "&numOfRows=10"; 
        url2 += "&pageNo=1"; 
        url2 += "&_type=json";       
 //--------------------        
        ...
        if(httpCode > 0) { 
          ...
          if(httpCode == HTTP_CODE_OK) { 
            ...
            JsonObject& root2 = jsonBuffer.parseObject(payload2.c_str()); 
...
/*
            ...
            JsonArray& mArray2 = root2["response"]["body"]["items"]["item"];
            
            JsonObject& wdata = jsonBuffer.createObject(); 
            JsonObject& wdata2 = jsonBuffer.createObject();
            ...
            //----------------동네예보-
            for(JsonArray::iterator it=mArray2.begin(); it!=mArray2.end(); ++it) { 
               JsonObject& item = *it; 
               const char* key = item["category"]; 
               const char* val = item["fcstValue"]; 
               const char* fcstDate = item["fcstDate"]; 
               const char* fcstTime = item["fcstTime"]; 
               sprintf(w_date, "%s%s", fcstDate, fcstTime); 
               wdata2[key] = val; 
               Serial.printf("%s %s %s\n",w_date,key,val); 
            }
            //-----------동네예보끝--
            ...
            int w_sky = wdata2["SKY"]; // 하늘 맑음(1), 구름조금(2삭제), 구름많음(3), 흐림(4) 
            int w_pty = wdata["PTY"]; // 강수형태(PTY) 코드 : 없음(0), 비(1), 비/눈(2), 눈(3) ,소나기(4추가)
 ...
 //           int w_wd_t = (w_wd + 22.5)/45; 
            ...
            tft.fillRect(xpos_sky,ypos_sky-25,85,102+15,TFT_BLACK);//이전정보 지우기
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            switch (w_pty) { 
                case 1: strcpy(w_pty_str,"Rain");tft.pushImage(xpos_sky,ypos_sky,rainWidth,rainHeight,rain);tft.drawCentreString(w_rn1_t,xpos_sky+40,ypos_sky-20,2);break; 
                case 2: strcpy(w_pty_str,"Rain");tft.pushImage(xpos_sky,ypos_sky,rainWidth,rainHeight,rain);tft.drawCentreString(w_rn1_t,xpos_sky+40,ypos_sky-20,2);break; 
                case 3: strcpy(w_pty_str,"Snow");tft.pushImage(xpos_sky,ypos_sky,snowWidth,snowHeight,snow);break; 
                case 4: strcpy(w_pty_str,"Shower");tft.pushImage(xpos_sky,ypos_sky,rainWidth,rainHeight,rain);tft.drawCentreString(w_rn1_t,xpos_sky+40,ypos_sky-20,2);break;
                default:  
                  switch (w_sky) { 
                    case 1: strcpy(w_pty_str,"Clean");tft.pushImage(xpos_sky,ypos_sky,cleanWidth,cleanHeight,clean);break;
                    case 2: strcpy(w_pty_str,"Slight Cloud");tft.pushImage(xpos_sky,ypos_sky,cloudy1Width,cloudy1Height,cloudy1);break; 
                    case 3: strcpy(w_pty_str,"Cloudy");tft.pushImage(xpos_sky,ypos_sky,cloudy1Width,cloudy1Height,cloudy1);break; //기상1,3,4만 제공으로 변경됨
                    case 4: strcpy(w_pty_str,"Dark");tft.pushImage(xpos_sky,ypos_sky+10,cloudy2Width,cloudy2Height,cloudy2);break; 
 ...
 //           온도습도바람정보표시
            ...
            tft.pushImage(0,ypos_weather,tempWidth,tempHeight,temp);//온도계 아이콘 표시
            tft.pushImage(0,ypos_weather+56,windWidth,windHeight,wind);//바람 아이콘 표시
       ...
       //     xpos_weather=60;
            ...
            // tft.drawString("Potential for errors",0,110,1);
...
if(s.available()>0){
 ...
 /* 아두이노에서 구현
  ...
  int d=day(),h=hour(),m=minute();
            ...
            xpos_timeup+=tft.drawString(monthShortStr(month()),xpos_timeup,230,1);
            xpos_timeup+=tft.drawChar('.',xpos_timeup,230,1);
            if(d<10) xpos_timeup+=tft.drawChar('0',xpos_timeup,230,1); 
            xpos_timeup+=tft.drawNumber(d,xpos_timeup,230,1);
            xpos_timeup+=tft.drawString(", ",xpos_timeup,230,1);
            if(h<10) xpos_timeup+=tft.drawChar('0',xpos_timeup,230,1); 
            xpos_timeup+=tft.drawNumber(h,xpos_timeup,230,1);
            xpos_timeup+=tft.drawString("h ",xpos_timeup,230,1);
            if(m<10) xpos_timeup+=tft.drawChar('0',xpos_timeup,230,1); 
            xpos_timeup+=tft.drawNumber(m,xpos_timeup,230,1);
            xpos_timeup+=tft.drawChar('m',xpos_timeup,230,1);
*/
}
...
//    if(wifiMulti.run() == WL_CONNECTED) { 
        ...
        url += "stationName=동탄"; //영통동, 동탄 또는 인계동, 기흥,영등포구,고천동
...
//               wdata[] = {pm10grade,pm10,pm25grade,pm25,khigrade};
  ...
  //          tft.drawRoundRect(xpos_air-4,ypos_air-2,90,85,10,TFT_CYAN);//먼지정보 영역표시
              ...
              //PM10  정보표시
            switch (pm10grade) { 
                case 1: strcpy(pm10grade_str,"Good");tft.setTextColor(TFT_WHITE);tft.pushImage(xpos_air-5,ypos_air+55,goodWidth,normalHeight,good);tft.pushImage(xpos_air,ypos_air,nodustWidth,nodustHeight,nodust);break; 
                case 2: strcpy(pm10grade_str,"Normal");tft.setTextColor(TFT_PURPLE);tft.pushImage(xpos_air-5,ypos_air+55,normalWidth,goodHeight,normal);tft.pushImage(xpos_air,ypos_air,dustNWidth,dustNHeight,dustN);break; 
                case 3: strcpy(pm10grade_str,"Bad");tft.setTextColor(TFT_PURPLE);tft.pushImage(xpos_air-5,ypos_air+55,badWidth,badHeight,bad);tft.pushImage(xpos_air,ypos_air,dustWidth,dustHeight,dust);break; 
                case 4: strcpy(pm10grade_str,"Severe");tft.setTextColor(TFT_WHITE);tft.pushImage(xpos_air-5,ypos_air+55,severeWidth,severeHeight,severe);tft.pushImage(xpos_air,ypos_air,dust2Width,dust2Height,dust2);break;
            ...
            tft.drawString(pm10,xpos_air-60,ypos_air+55+2,4);//pm10 값 표시
 ...
 //           tft.drawCentreString(pm10grade_str,xpos_air-17,ypos_air+55+2,4);//먼지 그래이드 표시
            ...
            tft.drawCentreString(pm25,xpos_air+12,ypos_air+10+3,2);//pm2.5 값표시
            
            //PM종류 정보 표시
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            tft.drawString("PM2.5",xpos_air-3,ypos_air,1);
         //   tft.drawString("PM 2.5",xpos_air+10,ypos_air+10+5,1);
            tft.drawString("PM10",xpos_air,ypos_air+45,1);
