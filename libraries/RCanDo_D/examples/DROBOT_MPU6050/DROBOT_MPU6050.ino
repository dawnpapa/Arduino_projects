// 알칸도 사선주행 밸런싱 로봇 - 자이로/가속도 센서(MPU6050) 체크 (V1.0)
//
...
//
// MPU6050 센서를 쉴드에 꼽고 본 프로그램을 업로드하면 센서 전후 방향의 기울기를 시리얼 모니터(115200)를 통해 알 수 있습니다. 
// MPU6050 센서를 전후로 기울여 가면서 변화되는 값을 아두이노 시리얼 모니터를 통해 확인해 보세요. 
//

#include <Wire.h>                                      // I2C 통신 
#define RAD2GRAD 57.2957795
// #define autonomous                                  // 자율주행 초음파 센서 모듈 적용시 주석 해제
#define RcandoUltrasonic_I2C_ADDRESS 0x07              // 알칸도 자율주행 초음파센서 모듈 주소
uint8_t distance[3];                                   // 초음파 센서 장애물까지의 거리 
...
void setup() {
  ...
  MPU6050_setup();   // MPU6050 셋업
  delay(500);
  MPU6050_calibrate();   // MPU6050 캘리브레이션
  timer_old = micros();
}
...
void loop() {
  ...
  if (MPU6050_newData()) {
    MPU6050_read_3axis();
    dt = (timer_value - timer_old) * 0.000001;        // 경과시간 dt(초)
    timer_old = timer_value;
    float MPU_sensor_angle = MPU6050_getAngle(dt);    // MPU6050 기울기
...
void autonomousDrive() {
  Wire.requestFrom(RcandoUltrasonic_I2C_ADDRESS, 3);
//  for (int i = 0; i < 3; i++)
//    distance[i] = Wire.read();                 // distance[0], distance[1], distance[2] : 좌측코너, 앞쪽, 우측코너의 장애물까지 거리(Cm)
