...
*/
 //   if(wifiMulti.run() == WL_CONNECTED) { 
        ...
        url += "&nx=61"; 
        url += "&ny=119"; //화성시 반월동 61,119, 서초구양재동 61,125, 동백 63,120 신길3동 59,125
        url += "&numOfRows=100"; 
        url += "&pageNo=1"; 
        url += "&_type=json"; 
 ...
 //          drawIcon(little,5,0,littleWidth,littleHeight);//리틀존 대문이름
             drawIcon(happy,5,0,happyWidth,happyHeight);//행복한우리집 대문이름
 //            drawIcon(sojung,5,0,sojungWidth,sojungHeight);//
 ...
 //           온도습도바람정보표시
       ...
       //     xpos_weather=60;
            ...
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            xpos_weather+=tft.drawString("Updated at ",xpos_weather,102,1);
...
//    if(wifiMulti.run() == WL_CONNECTED) { 
        ...
        url += "stationName=동탄"; //영통동, 동탄 또는 인계동, 기흥,영등포구
