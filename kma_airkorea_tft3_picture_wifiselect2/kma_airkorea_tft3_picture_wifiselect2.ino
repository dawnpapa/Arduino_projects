...
void setup() { 
 ...
 /*  
  ...
  wifiMulti.addAP("AndroidHotspot3142", ""); 
...
//    tft.fillRect(xpos_date,ypos_date,268,29,TFT_BLACK); //이전정보지우기
    ...
    if (oss != ss) { // Redraw seconds time every second
      ...
      if (ss % 2) { // Flash the colons on/off
        tft.setTextColor(0x39C4);        // Set colour to grey to dim colon
        tft.drawChar(':', xcolon, ypos - 8, 8);     // Hour:minute colon
        xpos += tft.drawChar(':', xsecs, ysecs, 6); // Seconds colon
        tft.setTextColor(TFT_YELLOW);    // Set colour back to yellow
      }
      else {
        tft.drawChar(':', xcolon, ypos - 8, 8);     // Hour:minute colon
        xpos += tft.drawChar(':', xsecs, ysecs, 6); // Seconds colon
...
//----------------동네예보--
 ...
 //           int w_wd_t = (w_wd + 22.5)/45; 
           ...
           // tft.fillRect(xpos_sky,ypos_sky-25,85,102+15,TFT_BLACK);//이전정보 지우기
            ...
            if(w_pty!=0) tft.setTextColor(TFT_RED);
 ...
 ////////////////////   온도습도바람 정보표시///////////////////////////
  ...
  //          tft.fillRect(0,30,100,87,TFT_BLACK); //화면 지우고 다시 표시,시계영역/먼지영역 제외
            ...
            int xpos_weather=80,ypos_weather=10;//온습도 정보영역 시작부위
    ...
    //        tft.pushImage(0,ypos_weather+56,windWidth,windHeight,wind);//바람 아이콘 표시
            ...
            tft.setTextColor(TFT_BLUE);
            tft.drawRightString(w_temp_t, xpos_weather, ypos_weather, 4);tft.drawChar('o',xpos_weather+5,ypos_weather,1);tft.drawString("C",xpos_weather+10,ypos_weather,2);
            tft.drawRightString(w_reh_t,xpos_weather,ypos_weather+25,4);tft.drawString("%",xpos_weather+5,ypos_weather+25,2);                 
            tft.drawRightString(w_ws_t,xpos_weather,ypos_weather+50,4);tft.drawString("m/s",xpos_weather+5,ypos_weather+50,2); 
...
//    if(wifiMulti.run() == WL_CONNECTED) { 
 ...
 //       url += "stationName="+Dongne; //동네이름 스트링 변수로 지정
