...
#include <SPI.h>
//#include "broadcom.h" //image drawing
...
int antSelected=0; //default antenna,0=ANT1,1=ANT2,2=ANT3
...
//    BOXSIZE = tft.width();
    ...
    tft.setTextSize(2);
    tft.print("ASW SPI CONTROLLER",10,1);  

    tft.setTextSize(1);
    tft.setTextColor(WHITE);
    tft.print("JMS'S 5TH Edition 2020",85,370);
    tft.print("FBAR SEOUL KOREA",120,380);

///회사이미지 삽입
//    tft.pushColors(0,370,broadcomWidth,broadcomHeight,broadcom);
...
//버튼생성
    ...
    tft.setTextSize(3);
    tft.setTextColor(WHITE);
    tft.print("UP",190,160);
    tft.print("DN",190,290);
    tft.setTextSize(2);
    tft.setTextColor(RED,GREEN);
    tft.print("ANT",188,215);
    tft.setTextSize(3);
    tft.printNumI(antSelected+1,197,235);
...
//SPI start&setting
    ...
    //The SPI settings are applied at the begin of the transaction and SPI.endTransaction() doesn't change SPI settings. 
...
//INFO
        ...
        // mode 버튼선택
            ...
            if (xpos > 165 && xpos<230 && ypos>217 && ypos<269) { //ANT button
              antSelected=(antSelected+1)%3;
              tft.setTextSize(3);
              tft.setTextColor(RED,GREEN);
              tft.printNumI(antSelected+1,197,235);
              modeChange();
...
void prjChange(){
  ...
  //initial color menu
    tft.setColor(WHITE);
    tft.fillRoundRect(1,20,75,50);
    tft.fillRoundRect(80,20,170,50);
    tft.fillRoundRect(175,20,240,50);
    tft.fillRoundRect(175,60,240,90);

    
        tft.setTextColor(BLACK);
        tft.setTextSize(2);
  
        tft.print("PINOT2",5,30);   
        tft.print("HLS2-NA",85,30);  
        tft.print("ROW",195,30);  
  ...
  switch (prjSelected) {
    ...
    case 0: 
        ...
        tft.fillRoundRect(1,20,75,50); 
        tft.print("PINOT2",5,30);
        strcpy(mode1,"01. B1/3/40/32 nonCA");
        strcpy(mode2,"02. B7         nonCA");
        strcpy(mode3,"03. B41        nonCA");
        strcpy(mode4,"04. B25/66/30  nonCA");
        strcpy(mode5,"05. B1/3/40/32+ 7 CA");
        strcpy(mode6,"06. B1/3/40/32+ 41CA");
        strcpy(mode7,"07. B25/66/30+  7 CA");
        strcpy(mode8,"08. B25/66/30+  41CA");
        strcpy(mode9,"09. B11/21");
        strcpy(mode10,"10. B40A");
        strcpy(mode11,"11. B34/39/41");
        strcpy(mode12,"12. NA");
        strcpy(mode13,"13. NA");
        strcpy(mode14,"14. NA");
        strcpy(mode15,"15. NA");
