...
#define NBR_MTX 4
LedControl lc=LedControl(11,13,10, NBR_MTX); //Din,Clck,Cs
...
void setup() {
  ...
  // 열(가로) LED  8개 제어
  lc.setRow(0,7,iconHeart[0]);
  lc.setRow(0,6,iconHeart[1]);
  lc.setRow(0,5,iconHeart[2]);
  lc.setRow(0,4,iconHeart[3]);
  lc.setRow(0,3,iconHeart[4]);
  lc.setRow(0,2,iconHeart[5]);
  lc.setRow(0,1,iconHeart[6]);
  lc.setRow(0,0,iconHeart[7]);
  delay(1000);
  lc.clearDisplay(0);

  // 문자열 출력
  lc.writeString(3,"Hola Mundo");
  delay(5000);
  lc.clearAll();
...
void loop() { 
  ...
  stringCounter++;
  if (stringCounter>37) stringCounter=0;
...
void scrollLeft(char ch){
  ...
  for (int scroll =0; scroll<6; scroll++) {
     for (int i=scroll; i<6;i++) {
        lc.setColumn(0,i-scroll, alphabetBitmap[pos][i]);
    } 
    delay(300);
    lc.clearDisplay(0);
  }
...
void scrollRight(char ch){
  ...
  for (int scroll =0; scroll<8; scroll++) {
     for (int i=0; i<6;i++) {
        if (scroll+i<8) lc.setColumn(0, scroll+i, alphabetBitmap[pos][i]);
    } 
    delay(300);
    lc.clearDisplay(0);
  }
