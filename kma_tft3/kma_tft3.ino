...
uint8_t hh = hour(), mm = minute(), ss = second(); 
 ...
 //   tft.setCursor(0,120);
    ...
    xpos_date+=tft.drawString(dayStr(weekday()),xpos_date,115,4);tft.fillRect(xpos_date,115,200,35,TFT_BLACK); //요일뒷부분지우기
 //   tft.print(year());tft.print(".");tft.print(month());tft.print(".");tft.print(day());tft.print(" ");tft.print(dayShortStr(weekday()));//tft.print(dayShortStr(weekday()));
...
//  tft.setTextSize(3);
 ...
 //           tft.setCursor(0,1);
            tft.fillRect(0,0,320,114,TFT_BLACK); //화면 지우고 다시 표
            int xpos_weather=200,ypos_weather=0;
            
            tft.setTextColor(TFT_GREEN,TFT_BLACK);
            tft.drawString("Temperature: ",0,ypos_weather,4);
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            
            xpos_weather+=tft.drawFloat(w_temp, 1, xpos_weather, ypos_weather, 4);
            xpos_weather+=tft.drawChar('o',xpos_weather,0,1);
            xpos_weather+=tft.drawString("C   ",xpos_weather,0,4);
            xpos_weather=200;
            
            tft.setTextColor(TFT_GREEN,TFT_BLACK);
            tft.drawString("Humidity: ",0,25,4);
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            xpos_weather+=tft.drawNumber(w_reh,xpos_weather,25,4);
            tft.drawString("%  ",xpos_weather,25,4);
            xpos_weather=200;
            
            tft.setTextColor(TFT_GREEN,TFT_BLACK);
            tft.drawString("Wind:      ",0,50,4);
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            xpos_weather+=tft.drawFloat(w_ws,1,xpos_weather,50,4); 
            xpos_weather+=tft.drawString("m/s   ",xpos_weather,50,4); 
            xpos_weather=107;           
  
            tft.setTextColor(TFT_GREEN,TFT_BLACK);
            xpos_weather+=tft.drawString("Sky: ",0,75,4);
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            if(w_pty!=0) tft.setTextColor(TFT_RED,TFT_BLACK);
            xpos_weather+=tft.drawString(w_pty_str,xpos_weather,75,4);
            xpos_weather=0;
            
            tft.setTextColor(TFT_BLUE,TFT_BLACK);
            xpos_weather+=tft.drawString("Updated at  Mainly:",xpos_weather,105,1);
            xpos_weather+=tft.drawString(w_date,xpos_weather,105,1);
            xpos_weather+=tft.drawString("  Shortly:",xpos_weather,105,1);
            xpos_weather+=tft.drawNumber(hour(),xpos_weather,105,1);xpos_weather+=tft.drawString("h ",xpos_weather,105,1);
            if(minute()<10) xpos_weather+=tft.drawChar('0',xpos_weather,105,1); xpos_weather+=tft.drawNumber(minute(),xpos_weather,105,1);tft.drawChar('m',xpos_weather,105,1);
