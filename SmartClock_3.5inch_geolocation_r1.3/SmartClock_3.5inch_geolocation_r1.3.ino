...
//variable setting
uint32_t titleColor = 0x2167; //background color of title region
...
uint32_t hotColor30 = 0xE1C1;//background color of tempature region when hot
uint32_t hotColor28 = 0xF480;
uint32_t mildColor = 0x35E8;
uint32_t mildColor20 = 0x0790;//background color of tempature region 
uint32_t coldColor10 = 0x04DE;//background color of tempature region when cold
uint32_t coldColor0 = 0x09DC;//background color of tempature region when cold
...
void setup() { 
  ...
  tft.pushImage(5,3,jeewonWidth,jeewonHeight,jeewon);//지원이화이팅 대문이름  
...
void time_display(){
  ...
  if (hh != ohh) { //1시간에 한번씩
    ...
    ///시간에 따른 디스플레이 밝기 조절
    if (hh == 23) analogWrite(D1,120);
    else if (hh >= 0 & hh <6) analogWrite(D1,80);
...
void check_kma(){
   //           온도습도바람정보표시
  int xtemp=150,ytemp=198,xtempEnd=220+15,ytempEnd=240;//온도정보영역
  int xhumid=260,yhumid=ytemp,xhumidEnd=318+15,yhumidEnd=ytempEnd;//습도정보영역
  ...
  tft.drawString("HUMIDITY",295,170);
...
void check_air(){
  ...
  switch (pm10Grade1h) { 
      ...
      default: tft.setTextColor(TFT_WHITE);
        pm10Value="--";
        tft.pushImage(xpos_air10,ypos_air10,dust10naWidth,dust10naHeight,dust10na);
        break;
  ...
  switch (pm25Grade1h) { 
      ...
      default:tft.setTextColor(TFT_WHITE);
        pm25Value="--";
        tft.pushImage(xpos_air25,ypos_air25,dust10naWidth,dust10naHeight,dust10na);
        break;
    }
