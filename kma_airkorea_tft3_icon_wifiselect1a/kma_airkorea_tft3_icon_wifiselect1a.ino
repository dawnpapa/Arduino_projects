...
//----------------동네예보--
 ...
 //--------------------        
        ...
        if(httpCode > 0) { 
          ...
          if(httpCode == HTTP_CODE_OK) { 
            ...
            int w_sky = wdata2["SKY"]; // 하늘 맑음(1), 구름조금(2), 구름많음(3), 흐림(4) 
 ...
 //          drawIcon(little,5,0,littleWidth,littleHeight);//리틀존 대문이름
             drawIcon(happy2,5,0,happy2Width,happy2Height);//행복한우리집 대문이름
  //          drawIcon(joonyoung,5,0,joonyoungWidth,joonyoungHeight);//준영이네
  //           drawIcon(sojung,5,0,sojungWidth,sojungHeight);//소정이네
            ...
            tft.setTextColor(TFT_WHITE,TFT_BLACK);
            switch (w_pty) { 
                case 1: strcpy(w_pty_str,"Rain");drawIcon(rain,xpos_sky,ypos_sky,rainWidth,rainHeight);tft.drawCentreString(w_rn1_t,xpos_sky+40,ypos_sky-20,2);break; 
                case 2: strcpy(w_pty_str,"Rain");drawIcon(rain,xpos_sky,ypos_sky,rainWidth,rainHeight);tft.drawCentreString(w_rn1_t,xpos_sky+40,ypos_sky-20,2);break; 
                ...
                default:  
                  switch (w_sky) { 
                    ...
                    case 3: strcpy(w_pty_str,"Mostly Cloud");drawIcon(cloudy2,xpos_sky,ypos_sky+10,cloudy2Width,cloudy2Height);break; 
                    case 4: strcpy(w_pty_str,"Dark Cloud");drawIcon(cloudy2,xpos_sky,ypos_sky+10,cloudy2Width,cloudy2Height);break; 
...
//               wdata[] = {pm10grade,pm10,pm25grade,pm25,khigrade};
  ...
  //          tft.drawRoundRect(xpos_air-4,ypos_air-2,90,85,10,TFT_CYAN);//먼지정보 영역표시
              ...
              //PM10  정보표시
            switch (pm10grade) { 
                case 1: strcpy(pm10grade_str,"Good");tft.setTextColor(TFT_WHITE);drawIcon(good,xpos_air-5,ypos_air+55,goodWidth,normalHeight);drawIcon(nodust,xpos_air,ypos_air,nodustWidth,nodustHeight);break; 
                case 2: strcpy(pm10grade_str,"Normal");tft.setTextColor(TFT_PURPLE);drawIcon(normal,xpos_air-5,ypos_air+55,normalWidth,goodHeight);drawIcon(dustN,xpos_air,ypos_air,dustNWidth,dustNHeight);break; 
                case 3: strcpy(pm10grade_str,"Bad");tft.setTextColor(TFT_PURPLE);drawIcon(bad,xpos_air-5,ypos_air+55,badWidth,badHeight);drawIcon(dust,xpos_air,ypos_air,dustWidth,dustHeight);break; 
                case 4: strcpy(pm10grade_str,"Severe");tft.setTextColor(TFT_WHITE);drawIcon(severe,xpos_air-5,ypos_air+55,severeWidth,severeHeight);drawIcon(dust2,xpos_air,ypos_air,dust2Width,dust2Height);break;
